...
//--------------------------------------------------------------------------------------------------
void oqpsk_demodulator::start(rx_thread_data_t *rx_thread_data_, rx_psdu_t *rx_sap_)
{
    stop();
    is_started = true;
    rx_sap_->ready = true;
...
//--------------------------------------------------------------------------------------------------
void oqpsk_demodulator::work(rx_thread_data_t *rx_thread_data_, rx_psdu_t *rx_sap_)
{
    ...
    while (!rx_thread_data->stop_demodulator){
        ...
        if(rx_thread_data->ready){
            ...
            rx_thread_data->set_mode = rx_sap_->mode;
...
//                if(rx_sap_->rssi > -85)fprintf(stderr, "oqpsk_demodulator::rssi = %f\n", rx_sap_->rssi);
...
//                    fprintf(stderr, "ed %f  %f  %f\n", level_thr, energy_detect, energy_detect_2);
        ...
        case frame_delimiter_phase_synchronisation:
            ...
            if(idx_i == LEN_CHIP - 1){
                ...
                callback_device->preamble_correlation_callback(LEN_PREAMBLE, preamble_correlation_buffer);
        ...
        case frame_lenght:
            ...
            // check frame lenght
            if(state == payload){
                if(len_symbols > MaxPHYPacketSize){
                    fprintf(stderr, "oqpsk_demodulator::demodulator len_symbols > MaxPHYPacketSize\n");
