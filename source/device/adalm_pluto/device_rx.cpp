...
device_rx::~device_rx()
{
    fprintf(stderr, "device_rx::~device_rx() start\n");
    fprintf(stderr, "device_rx::~device_rx() stop\n");
}
//-----------------------------------------------------------------------------------------
void device_rx::start(iio_device *rx_, rx_thread_data_t *rx_thread_data_)
{
    ...
    fprintf(stderr, "iio_device_set_kernel_buffers_count= %d success=%d\n", 2, e);
    rx_buffer = iio_device_create_buffer(rx_, RX_PLUTO_LEN_BUFFER, false);
    is_started = true;
    work(rx_thread_data_);
}
//-----------------------------------------------------------------------------------------
void device_rx::work(rx_thread_data_t *rx_thread_data_)
{
//#define RETING
    static int c = 0;
    ...
    while(!rx_thread_data->stop){
        ...
        if (rx_thread_data->mutex.try_lock()){
            ...
            ++c;
            fprintf(stderr, "device::rx_fill_data : rx_buffer skip buffer %d\n", c);
        }
    ...
    fprintf(stderr, "device_rx::work() finish\n");
}
//-----------------------------------------------------------------------------------------
void device_rx::stop()
{
