...
//    std::cout << "rxBatchSize= " << rxBatchSize << std::endl;
...
//        SetOSThreadPriority(ThreadPriority::NORMAL, ThreadPolicy::REALTIME, &txThread);
        SetOSThreadPriority(ThreadPriority::HIGHEST, ThreadPolicy::REALTIME, &txThread);
    }
    return 0;
}
...
//    std::cout << "packetsToBatch= " << (uint16_t)packetsToBatch << std::endl;
    const uint32_t bufferSize = packetsToBatch*sizeof(FPGA_DataPacket);
    std::vector<int> handles(buffersCount, 0);
