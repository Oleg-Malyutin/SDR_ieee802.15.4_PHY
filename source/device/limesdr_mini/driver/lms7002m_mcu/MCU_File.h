	...
	void ReadBin(unsigned long limit)
	{
		...
		while (!feof(m_file))
		{
			...
			if (m_chunks.back().m_bytes.size() > limit + 1)
			{
                ...
                std::cout << "Ignoring data above address space!\n";
	...
	void ReadHex(unsigned long limit)
	{
		...
		while (!feof(m_file))
		{
			...
			if (!formatDetected)
			{
				...
				{
                    std::cout << "Ignoring garbage line!\n";
					continue;
				}
				if (szLine[0] == 'S')
				{
					intel = false;
                    std::cout << "Detected S-Record\n";
				}
				else
				{
					intel = true;
                    std::cout << "Detected intel hex file\n";
				}
				formatDetected = true;
			}
			...
			if (intel)
			{
				...
				case 0:
					...
					}
					{
						...
						for (i = 0; i < dataBytes; ++i)
						{
							...
							{
                                std::cout << "Ignoring data above address space!\n";
                                std::cout << "Data address: " << addressBase + startAddress + i;
                                std::cout << " Limit: " << limit << "\n";
								if (!m_chunks.back().m_bytes.size())
								{
									m_chunks.pop_back();
				...
				case 1:
					...
					if (dataBytes != 0)
					{
                        std::cout << "Warning: End of file record not zero length!\n";
					}
					if (startAddress != 0)
					{
                        std::cout << "Warning: End of file record address not zero!\n";
					}
					endSeen = true;
					break;
				...
				case 3:
					...
					if (dataBytes != 4)
					{
                        std::cout << "Warning: Length field must be 4 in start segment address record!\n";
					}
					if (startAddress != 0)
					{
                        std::cout << "Warning: Address field must be zero in start segment address record!\n";
					}
					if (dataBytes == 4)
					{
						unsigned long ssa;
						...
						sprintf(ssaStr, "%08lX\n", ssa);
                        std::cout << "Segment start address (CS/IP): ";
                        std::cout << ssaStr;
					}
					break;
				...
				case 5:
					...
					if (dataBytes != 4)
					{
                        std::cout << "Warning: Length field must be 4 in start linear address record!\n";
					}
					if (startAddress != 0)
					{
                        std::cout << "Warning: Address field must be zero in start linear address record!\n";
					}
					if (dataBytes == 4)
					{
						unsigned long lsa;
						...
						sprintf(lsaStr, "%08lX\n", lsa);
                        std::cout << "Linear start address: ";
                        std::cout << lsaStr;
					}
					break;

				default:
                    std::cout << "Waring: Unknown record found!\n";
				}
			}
			else
			{
				...
				case '3':
					// Data record
					{
						...
						for (i = uint8_t(type - '1'); uint8_t(i + 3) < count; ++i)
						{
							...
							{
                                std::cout << "Ignoring data above address space!\n";
                                std::cout << "Data address: " << startAddress + i;
                                std::cout << " Limit: " << limit << "\n";
								if (!m_chunks.back().m_bytes.size())
								{
									m_chunks.pop_back();
				...
				case '9':
                    ...
                    std::cout << "Ignoring start address record!\n";
					break;

				default:
                    std::cout << "Unknown record found!\n";
				}
			}
		}
		if (intel && !endSeen)
		{
            std::cout << "No end of file record!\n";
		}
		if (!m_chunks.size())
		{
			throw "No data in file!\n";
		}
        std::vector<MemBlock>::iterator	vi;
		m_top = 0;
		for (vi = m_chunks.begin(); vi < m_chunks.end(); vi++)
		{
            unsigned long t = vi->m_startAddress + vi->m_bytes.size() - 1;
            #if !(defined(max)) && _MSC_VER
                // VC fix
                m_top = __max(m_top, t);
            #else
                m_top = std::max(m_top, t);
            #endif
		}
	}
