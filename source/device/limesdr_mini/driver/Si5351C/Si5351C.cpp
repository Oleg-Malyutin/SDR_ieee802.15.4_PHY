...
*/
bool Si5351C::LoadRegValuesFromFile(std::string FName)
{
    std::fstream fin;
...
void Si5351C::FindVCO(Si5351_Channel *clocks, Si5351_PLL *plls, const unsigned long Fmin, const unsigned long Fmax)
{
    ...
    std::map< unsigned long, int> availableFrequenciesPLLA; //all available frequencies for VCO
    std::map< unsigned long, int> availableFrequenciesPLLB; //all available frequencies for VCO
    ...
    if(clocks[6].powered || clocks[7].powered)
    {
        ...
        for (std::set<unsigned long>::iterator it6=clk6freqs.begin(); it6!=clk6freqs.end(); ++it6)
        {
            for (std::set<unsigned long>::iterator it7=clk7freqs.begin(); it7!=clk7freqs.end(); ++it7)
            {
                if(*it6 == *it7)
                {
                    canShare = true;
        ...
        if(canShare) //assign PLLA for both clocks
        {
            ...
            pllAused = true;
            for (std::set<unsigned long>::iterator it=sharedFreqs.begin(); it!=sharedFreqs.end(); ++it)
            {
                availableFrequenciesPLLA.insert( std::pair<unsigned long, int> (*it, 0) );
            }
        }
        else //if clocks 6 and 7 can't share pll, assign pllA to clk6 and pllB to clk7
        {
            if(!clk6satisfied)
            {
                ...
                for (std::set<unsigned long>::iterator it6=clk6freqs.begin(); it6!=clk6freqs.end(); ++it6)
                {
                    availableFrequenciesPLLA.insert( std::pair<unsigned long, int>(*it6, 0) );
                }
            }
            if(!clk7satisfied)
            {
                ...
                for (std::set<unsigned long>::iterator it7=clk7freqs.begin(); it7!=clk7freqs.end(); ++it7)
                {
                    availableFrequenciesPLLB.insert( std::pair<unsigned long, int> (*it7, 0) );
                }
            }
        }
    }
    ...
    if( availableFrequenciesPLLA.size() == 0 && !pllAused)
    {
        for(int i=0; i<6; ++i)
        {
            ...
            while(freq >= Fmin && freq <= Fmax)
            {
                ...
                availableFrequenciesPLLA.insert( std::pair<unsigned long, int> (freq, 0));
    ...
    for (std::map<unsigned long, int>::iterator it=availableFrequenciesPLLA.begin(); it!=availableFrequenciesPLLA.end(); ++it)
    {
        for(int i=0; i<8; ++i)
        {
    ...
    if( availableFrequenciesPLLB.size() == 0 && !pllBused)
    {
        for(int i=0; i<6; ++i)
        {
            ...
            while(freq >= Fmin && freq <= Fmax)
            {
                availableFrequenciesPLLB.insert( std::pair<unsigned long, int> (freq, 0));
                freq += clocks[i].outputFreqHz;
            }
        }
    }
    ...
    for (std::map<unsigned long, int>::iterator it=availableFrequenciesPLLB.begin(); it!=availableFrequenciesPLLB.end(); ++it)
    {
        for(int i=0; i<8; ++i)
        {
            if(clocks[i].outputFreqHz == 0 || !clocks[i].powered)
