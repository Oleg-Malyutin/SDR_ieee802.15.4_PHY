...
int LMS7002M::SaveConfig(const char* filename)
{
    ...
    fout << "sxt_ref_clk_mhz=" << this->GetReferenceClk_SX(Tx) / 1e6 << std::endl;
    fout << "sxr_ref_clk_mhz=" << this->GetReferenceClk_SX(Rx) / 1e6 << std::endl;
    fout.close();
    return 0;
}
...
int LMS7002M::SetFrequencySX(bool tx, float_type freq_Hz, SX_details* output)
{
    ...
    if  (useCache && tuning_cache_sel_vco.count(freq_Hz) > 0)
    {
        ...
        std::this_thread::sleep_for(std::chrono::microseconds(50)); // probably no need for this as the interface is already very slow..
...
int LMS7002M::RegistersTest(const char* fileName)
{
    ...
    std::vector<MemorySection> modulesToCheck = { AFE, BIAS, XBUF, CGEN, BIST, CDS, TRF, TBB, RFE, RBB, SX,
